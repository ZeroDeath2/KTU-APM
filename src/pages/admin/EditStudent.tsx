...
const EditStudent: React.FC = () => {
    ...
    const fetchStudentData = async () => {
      try {
        ...
        const studentData = {
          ...
          assignedStaffId: '1', // Example assigned staff
        };
        setFormData(studentData);
        // Set the staff search field to show the assigned staff's name
        const assignedStaff = staffList.find(staff => staff.id === studentData.assignedStaffId);
        if (assignedStaff) {
          setStaffSearch(assignedStaff.name);
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Updating student with data:', { ...formData, studentId });
      
      // Navigate back to students list after successful update
      navigate('/admin/students');
    } catch (error) {
    ...
    <DashboardLayout>
      <div className="mb-8 flex items-center">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-2">Update student information and staff assignment.</p>
        </div>
      </div>
